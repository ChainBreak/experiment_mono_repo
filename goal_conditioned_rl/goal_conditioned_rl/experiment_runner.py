...
def run_all_experiments(config: GhostConfig) -> None:
    for experiment_config in config["experiments"]:
        ExperimentRunner(experiment_config).run()
...
class ExperimentRunner:
    def __init__(self, config: GhostConfig):
        ...
        environment_id = config["environment"].get("id", "Acrobot-v1")
        probe_environment = gymnasium.make(environment_id)
        observation_dimension = probe_environment.observation_space.shape[0]
        action_dimension = probe_environment.action_space.n
        probe_environment.close()

        self.environment_id = environment_id
        self.data_directory = config["runner"].get("data_directory", "recorded_episodes")
        self.video_directory = config["runner"].get("video_directory", "videos")
